...
class CalculatorViewModel : ViewModel() {
    ...
    fun onButtonClick(button: CalculatorButton) {
        ...
        val lastValue =
            if (_uiState.value.enteredEquation.isNotEmpty()) _uiState.value.enteredEquation.last() else ' '
        ...
        when (button.category) {
            ...
            else -> addToEquation(button.text)
        }
    ...
    private fun addToEquation(value: String) {
        ...
        _uiState.update {
            it.copy(enteredEquation = it.enteredEquation + value)
        }
    }

    private fun changeLastValue(value: String) {
        _uiState.update {
            it.copy(enteredEquation = it.enteredEquation.dropLast(1) + value)
        }
    }

    fun clearLast() {
        val lastValue =
            (if (_uiState.value.enteredEquation.isNotEmpty()) _uiState.value.enteredEquation.last() else ' ').toString()
        ...
        _uiState.update { it.copy(enteredEquation = it.enteredEquation.dropLast(1)) }
